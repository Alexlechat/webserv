...
ServerConfig	ConfigParser::_parseServer(void)
{
	...
	while (_current() != "}")
	{
		...
		if (key == "listen") { serverConfig.setPort(_consume()); _expect(";"); }
		else if (key == "server_name") { serverConfig.setServerName(_consume()); _expect(";"); }
...
LocationConfig	ConfigParser::_parseLocation(const std::string& path)
{
	...
	while (_current() != "}")
	{
		...
		if (key == "root") { locationConfig.setRoot(_consume()); _expect(";"); }
		else if (key == "index") { locationConfig.setIndex(_consume()); _expect(";"); }
		else if (key == "methods")
		{
			while (_current() != ";")
		...
		else if (key == "autoindex") { locationConfig.setAutoindex(_consume()); _expect(";"); }
		else if (key == "upload_store") { locationConfig.setUploadStore(_consume()); _expect(";"); }
		else if (key == "redirect") { locationConfig.setRedirect(_consume(), _consume()); _expect(";"); }
		else if (key == "cgi_extension") { locationConfig.setCgiExtensions(_consume(), _consume()); _expect(";"); }
...
{
	if (_consume() != token)
		throw ParseException("Unexpected token found: " + token);
}
